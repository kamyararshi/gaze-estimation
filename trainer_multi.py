...
def ddp_setup(rank, world_size):
    ...
    """
    os.environ["MASTER_ADDR"] = 'localhost' # Refers to the IP of the machine running the rank 0 process
...
def load_train_objs(data_path, lr, total_epochs, batch_size, save_every, pretrained):
    ...
    logger.info("optimizer building")
    optimizers={}
...
def prepare_dataloader(dataset: Dataset, batch_size: int):
    return DataLoader(
        ...
        pin_memory=True,
        shuffle=False, # Since we are passing a sampler below
        sampler=DistributedSampler(dataset)
    )

## main function ---------------------------------------------------------------------------
def main(rank: int, world_size: int, log_dir, config):
    ddp_setup(rank, world_size)
    
    dataset, eval_dataset, model, optimizers = load_train_objs(**config["train_params"])
    loss = load_losses(**config["loss_params"], rank=rank)
    ...
    with Trainer(model, train_loader, eval_loader, optimizers, loss, log_dir, rank, train_params["save_every"]) as trainer:
...
if __name__ == "__main__":
    ...
    mp.spawn(main, args=(world_size, log_dir, configs), nprocs=world_size) # We don't need to pass "rank" argument here
    # main(args.gpu_id, log_dir, configs) # Do not need ths anymore